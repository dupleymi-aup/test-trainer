<div align="center">

# Тренажёр тестирования v4.0 / TestTrainer v4.0

### Интерактивная платформа для изучения методов тестирования ПО
### Interactive Platform for Software Testing Practice

[![Next.js](https://img.shields.io/badge/Next.js-15-black?logo=next.js)](https://nextjs.org/)
[![TypeScript](https://img.shields.io/badge/TypeScript-5-blue?logo=typescript)](https://www.typescriptlang.org/)
[![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-4-38bdf8?logo=tailwindcss)](https://tailwindcss.com/)
[![shadcn/ui](https://img.shields.io/badge/shadcn%2Fui-new--york-black)](https://ui.shadcn.com/)
[![Prisma](https://img.shields.io/badge/Prisma-6-2d3748?logo=prisma)](https://www.prisma.io/)
[![Recharts](https://img.shields.io/badge/Recharts-2-ff7300)](https://recharts.org/)
[![License](https://img.shields.io/badge/License-Proprietary-red)](./LICENSE)

---

**Автор / Author:** Дуплей Максим Игоревич / Dupley Maxim Igorevich

**Интеллектуальная собственность / Intellectual Property:** Дуплей Максим Игоревич / Dupley Maxim Igorevich

</div>

---

## 📖 Документация / Documentation

| Язык / Language | Файл |
|-----------------|------|
| 🇷🇺 Русский | [README_RU.md](./README_RU.md) |
| 🇬🇧 English | [README_EN.md](./README_EN.md) |

---

## 🚀 Быстрый старт / Quick Start

```bash
git clone https://github.com/QuadDarv1ne/test-trainer.git
cd test-trainer
npm install
npx prisma db push
npm run dev
```

Приложение доступно по адресу [http://localhost:3000](http://localhost:3000)

---

## 👤 Автор / Author

**Дуплей Максим Игоревич / Dupley Maxim Igorevich**

---

## 📄 Лицензия / License

Данный проект является интеллектуальной собственностью Дуплей Максима Игоревича.
Условия использования описаны в файле [LICENSE](./LICENSE).

This project is the intellectual property of Dupley Maxim Igorevich.
Terms of use are described in the [LICENSE](./LICENSE) file.

---

<div align="center">

**Тренажёр тестирования v4.0** — © 2025–2026 Дуплей Максим Игоревич / Dupley Maxim Igorevich

</div>
